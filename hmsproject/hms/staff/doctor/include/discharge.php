...
function acceptDischargeRequest($p_id)
{
    ...
    $result2 = mysqli_query($con, "UPDATE discharge SET agreed_discharge_time = '$ldate' WHERE p_id = '$p_id' AND agreed_discharge_time IS NULL AND disagree_discharge_time IS NULL");
    $query   = "UPDATE discharge SET doctor_status = '1' WHERE p_id = '$p_id' AND wardAdmin_status = '1'";
    $result  = mysqli_query($con, $query);
    if ($result & $result2) {
        return true;
    } else {
...
$sql  = mysqli_query($con, "SELECT * FROM patienttb WHERE P_ID='P00003'");
